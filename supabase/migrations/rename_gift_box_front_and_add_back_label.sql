-- Rename "Gift Box Labels" → "Gift Box Front Label" (keeps category id / existing SKUs)
-- and add Gift Box Back Label as a second per-bottle BOM component.
--
-- Front slot stays gift_box_label_sku_id (already on v_portal_boms).
-- Back column is appended to the view — do not DROP CASCADE.

UPDATE dry_goods_categories
SET name = 'Gift Box Front Label'
WHERE lower(name) = 'gift box labels';

INSERT INTO dry_goods_categories (name, sort_order)
SELECT 'Gift Box Back Label', 25
WHERE NOT EXISTS (SELECT 1 FROM dry_goods_categories WHERE lower(name) = 'gift box back label');

ALTER TABLE boms ADD COLUMN IF NOT EXISTS gift_box_back_label_sku_id text;
ALTER TABLE client_bom_submissions ADD COLUMN IF NOT EXISTS gift_box_back_label_sku_id text;
